...
try {
    ...
    String commentBody = props.notNull('commentBody')
    String commentAuthor = props.optional('commentAuthor')
    long commentAuthorId = 0
    if (OctaneHelper.isNotEmpty(commentAuthor)) {
        commentAuthorId = Long.parseLong(commentAuthor)
    }
    ...
    oClient.updateWorkItems(wiIds, phaseName, commentBody, commentAuthorId, failMode)
